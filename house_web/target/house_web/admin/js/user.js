        ...
        columns: [[
            ...
            {field: 'name', title: '区域名称', width: 100, align: 'left'},
            {
                field: 's', title: '操作', width: 100, formatter: function (value, row, index) {
                    //var str = "<a href='DeleteUsers?id=" + row.id + "'>删除</a>|<a href=''>修改</a>"
                    //上面的操作是同步发送请求 不会跳转页面
...
function SaveDialog() {
    //1.获取数据，发送异步请求实现添加
    //$.post("addUsers",给服务器传参,function(data){},"json");
    /* $.post("addUsers",{"name":$("#name2").val()},function(data){
         alert(data);
     },"json");*/
...
function updateDialog() {
    ...
    if (SelectRows.length == 1) {   //如果复选框选择的值为1 （选中一个） 就执行
        ...
        $.post("SelectUsersOne", {"id": SelectRows[0].id}, function (data) {
...
function deleteDis(obj) {
    ...
    $.messager.confirm("提示框", "确认是否需要删除？", function (r) {  //这里的r 代表true的意思
        if (r) {
            ...
            $.post("DeleteUsers", {"id": obj}, function (data) {
                if (data.result == 1) {
                    //实现datagrid的刷新
                    $('#dg').datagrid('reload');
                } else {
...
//实现Datagrid的搜索功能
function Search() {
    //实现搜索查询    这个刷新的代码具可以填写查询条件的功能  而且她能做到自动提交  并且页码页大小也能自动提交实现分页

    //datagrid的load方法是重新加载，他会将查询条件 随着页码 页大小
    //一起发送到指定的服务器地址进行查询     url: 'getUsers',

    //$("#dg").datagrid("load", "传查询条件:{键:值,键:值}");
    //这个键就是封装JavaBean里面的变量 值就是通过ID取出来的.val值
